...
def visit_level(t, level, act):
    ...
    if level == 0:
        ...
        return sum([visit_level(c, level-1, act) for c in t.children])


def levelorder_visit_recursive(tree, act):
    """
    Visit Tree t in level order and act on its nodes.

    @param Tree tree: Tree to visit in level order
    ...
    """
    root = tree
    visited = visit_level(root, 0, act)
    level = 0
    while visited > 0:
        level += 1
        visited = visit_level(root, level, act)
